...
use crate::release::auto_ignore::{is_outside_tracking_scope, should_auto_ignore};
use crate::release::candidate::ReleaseCandidate;
use crate::release::languages::{includes, resolve_for_series};
use crate::release::matcher::{evaluate, resolve_threshold};
use crate::scheduler_handle::SharedSchedulerReconciler;
...
impl ReleasesRequestHandler {
    ...
    async fn handle_record(&self, request: &JsonRpcRequest) -> JsonRpcResponse {
        ...
        let candidate_volume_primary =
            crate::release::candidate::primary_value(candidate_volumes.as_ref()).map(|v| v as i32);
        let candidate_chapter_primary =
            crate::release::candidate::primary_value(candidate_chapters.as_ref());
        ...
        if !outcome.deduped {
            ...
            if outcome.row.state != ledger_state::ANNOUNCED {
                ...
                let series_title =
                    crate::release::announce::lookup_series_title(&self.db, outcome.row.series_id)
                        .await;
...
mod tests {
    use super::*;
    use crate::plugin::protocol::ReleaseSourceKind;
    use crate::release::candidate::{NumericSpan, SeriesMatch};
