...
//! tracing layer. Repository methods are instrumented at the method boundary
//! instead of wrapping raw SQL, so a single SeaORM call shows up as one span
//! tagged with the operation (`select`, `insert`, `update`, `delete`) and a
//! stable entity name (`book`, `series`, ...).
//!
...
//!
//! Lives in the `db` module because the only inputs to `db_system_str` are
//! SeaORM types — there is no observability-side dependency.
//!
...
mod tests {
    ...
    /// attributes. Callers can rely on the `db.<entity>.<operation>` naming and
    /// the `db.system`, `db.operation`, `otel.kind` fields being populated.
    #[tokio::test]
    async fn instrumented_repo_method_emits_named_span_with_semantic_conv_fields() {
        use super::super::repositories::UserRepository;
        use uuid::Uuid;
    ...
    /// with `cargo test --release -p codex-db -- --ignored bench_instrumentation_overhead --nocapture`
