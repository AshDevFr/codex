...
use crate::entities::{series_metadata, series_metadata::Entity as SeriesMetadata};
...
async fn emit_metadata_updated(
    ...
    let library_id = match crate::repositories::SeriesRepository::get_by_id(db, series_id).await {
...
mod tests {
    ...
    async fn make_series(db: &crate::Database, name: &str) -> Uuid {
        let library = LibraryRepository::create(
            db.sea_orm_connection(),
            "Test Library",
            "/test/path",
