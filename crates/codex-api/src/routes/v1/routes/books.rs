...
pub fn routes(_state: Arc<AppState>) -> Router<Arc<AppState>> {
    Router::new()
        ...
        // One entry, for correcting a single wrong read-through.
        .route(
            "/books/{book_id}/read-history/{completion_id}",
            delete(handlers::delete_book_read_history_entry),
        )
        ...
        .route(
            "/books/bulk/metadata",
            patch(handlers::bulk_patch_book_metadata),
        )
        .route(
            "/books/bulk/metadata/locks",
            put(handlers::bulk_update_book_locks),
        )
        .route("/books/bulk/tags", post(handlers::bulk_modify_book_tags))
        .route(
            "/books/bulk/genres",
            post(handlers::bulk_modify_book_genres),
        )
        // Book genre routes
