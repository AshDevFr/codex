...
use crate::permissions::{Permission, UserRole};
...
pub struct AppState {
    ...
    /// None when workers are disabled (`task.run_in_process: false`) or in test environments
    ...
    /// Application name surfaced by `GET /api/v1/info`.
    /// Read from `env!("CARGO_PKG_NAME")` by the binary at startup so the
    /// reported value tracks the binary crate, not this library crate.
    pub app_name: &'static str,
    /// Application version surfaced by `GET /api/v1/info`. Same plumbing as
    /// [`AppState::app_name`].
