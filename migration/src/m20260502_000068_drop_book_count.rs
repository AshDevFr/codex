...
//!
//! Hard removal. The new `total_volume_count` / `total_chapter_count` columns
//! and their locks are now the sole source of truth, written by
//! `MetadataApplier` and surfaced through every read site. The legacy column
//! is no longer read or written anywhere in the codebase, so we drop it to make
//! any leftover reference fail at compile or runtime.
//!
...
//! restoring from a pre-removal backup.
