...
import type { FileSystemEntry, BrowseResponse } from '@/types/api';
...
describe('AddLibraryModal', () => {
  ...
  it('should render form fields when opened', async () => {
    ...
    expect(modalContent.getByPlaceholderText('Enter library name')).toBeInTheDocument();
    expect(modalContent.getByPlaceholderText('Select a path...')).toBeInTheDocument();
  ...
  it('should select path and auto-generate library name', async () => {
    ...
    const modalContent = within(modal);
    const nameInput = await modalContent.findByPlaceholderText('Enter library name', {}, { timeout: 3000 });
    ...
    await waitFor(() => {
      ...
      const pathInput = modalContent.getByPlaceholderText('Select a path...') as HTMLInputElement;
      expect(pathInput.value).toBe('/home/user');

      const nameInput = modalContent.getByPlaceholderText('Enter library name') as HTMLInputElement;
  ...
  it('should show validation error when name is missing', async () => {
    ...
    const modal = await screen.findByRole('dialog', {}, { timeout: 3000 });
    const modalContent = within(modal);
    await modalContent.findByPlaceholderText('Enter library name', {}, { timeout: 3000 });

    // Don't fill in name or path, button should be disabled
    // The button is disabled when !libraryName || !selectedPath
    const createButton = modalContent.getByRole('button', { name: /create library/i });
  ...
  it('should navigate back to drives when breadcrumb is clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Select a drive or location to browse:')).toBeInTheDocument();
    });
  });
});
