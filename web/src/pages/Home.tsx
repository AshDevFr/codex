...
import { Container, Title, SimpleGrid, Card, Text, Group, Badge, Button, Stack, Loader, Center, Progress, ActionIcon, Menu, Modal } from '@mantine/core';
import { IconBooks, IconFolder, IconDots, IconEdit, IconTrash, IconScan, IconRadar, IconPlus } from '@tabler/icons-react';
...
export function Home() {
      ...
      (progress) => {
        ...
        if (progress.status === 'completed' || progress.status === 'failed') {
          ...
          if (progress.status === 'completed') {
            ...
            notifications.show({
              title: 'Scan failed',
              message: progress.error_message || 'An error occurred during scanning',
              color: 'red',
            });
          }
  ...
  const scanMutation = useMutation({
    mutationFn: ({ libraryId, mode }: { libraryId: string; mode: 'normal' | 'deep' }) =>
  ...
  return (
    <Container size="xl" py="xl">
        ...
        <Group justify="space-between">
          ...
          <Group gap="xs">
            <Menu shadow="md" width={200} position="bottom-end">
              <Menu.Target>
                <ActionIcon variant="light" size="lg" title="Scan All Libraries">
                  <IconScan size={20} />
                </ActionIcon>
              </Menu.Target>
        ...
        {libraries && libraries.length > 0 ? (
          <SimpleGrid cols={{ base: 1, sm: 2, lg: 3 }} spacing="lg">
            {libraries.map((library: Library) => {
              ...
              const isScanning = progress && (progress.status === 'pending' || progress.status === 'running');
                ...
                <Card key={library.id} shadow="sm" padding="lg" radius="md" withBorder>
                  <Stack gap="md">
                    <Group justify="space-between">
                      <Group gap="xs">
                        ...
                        <Menu shadow="md" width={200} position="bottom-start">
                          ...
                          <Menu.Dropdown>
                            <Menu.Item
                              leftSection={<IconScan size={16} />}
                              onClick={() => scanMutation.mutate({ libraryId: library.id, mode: 'normal' })}
                              disabled={isScanning}
                            >
                              Scan Library
                            </Menu.Item>
                            <Menu.Item
                              leftSection={<IconRadar size={16} />}
                              onClick={() => scanMutation.mutate({ libraryId: library.id, mode: 'deep' })}
                              disabled={isScanning}
                            >
                              Scan Library (Deep)
                            </Menu.Item>
                      ...
                      <Badge
                        color={library.scanningConfig?.enabled ? 'green' : 'gray'}
                      >
                    ...
                    {library.lastScannedAt && !isScanning && (
                      <Text size="xs" c="dimmed">
                        Last scan: {new Date(library.lastScannedAt).toLocaleString()}
                      </Text>
                    )}
                    ...
                    {isScanning && progress && (
                      <Stack gap="xs">
                        <Group justify="space-between">
                          <Text size="sm" fw={500}>
                            {progress.status === 'pending' ? 'Starting scan...' : 'Scanning...'}
                          </Text>
                          <Text size="sm" c="dimmed">
                        ...
                        {(progress.books_found > 0 || progress.series_found > 0) && (
                          <Text size="xs" c="dimmed">
                            Found {progress.books_found} books in {progress.series_found} series
                          </Text>
                        )}
                      </Stack>
                    )}
      ...
      >
        <Stack gap="md">
          <Text>
            Are you sure you want to delete <strong>{libraryToDelete?.name}</strong>?
          </Text>
          <Text size="sm" c="dimmed">
            This will remove the library from Codex. The files on disk will not be deleted.
          </Text>
