...
- **Command allowlist**: Only approved commands can be used to launch plugins (`node`, `npx`, `python`, `python3`, `uv`, `uvx`, and paths under `/opt/codex/plugins/`). Custom commands can be allowed via the `plugins.allowed_commands` setting, or `CODEX_PLUGINS__ALLOWED_COMMANDS='[deno, bun]'`.
